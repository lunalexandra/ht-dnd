...
;// CONCATENATED MODULE: ./src/js/SmallCard.js
class SmallCard {
  constructor(text) {
    this.text = text;
  }
...
;// CONCATENATED MODULE: ./src/js/TextArea.js
class TextArea {
  constructor(element) {
...
class Trello {
    ...
    addBtns.forEach((btn, index) => {
      btn.addEventListener("click", event => {
        ...
        addCardBtn.addEventListener("click", e => {
          e.preventDefault();
          if (input.value) {
            cardBody.insertAdjacentHTML("beforeend", this.addSmallCard(input.value));
  ...
  save(index) {
    localStorage.setItem(`cards_${index}`, JSON.stringify(this.cardsTextArr[index]));
  }
  load() {
    ...
    cards.forEach((el, index) => {
      ...
      if (savedCards) {
        ...
        savedCards.forEach(text => {
          ...
          el.querySelector(".card-body").insertAdjacentHTML("beforeend", smallCardMarkup);
        });
  ...
  onMouseMove(e) {
    ...
    const nextEl = elementsUnderCursor.find(el => el.classList.contains("small-card") && el !== this.draggedItem);
  ...
  onMouseUp(e) {
    ...
    const nextEl = elementsUnderCursor.find(el => el.classList.contains("small-card") && el !== this.draggedItem);
    const newParent = elementsUnderCursor.find(el => el.classList.contains("card-body"));
  ...
  updateCardsTextArr() {
    ...
    cardBodies.forEach((cardBody, index) => {
      ...
      smallCards.forEach(smallCard => {
        this.cardsTextArr[index].push(smallCard.textContent.replace("✖", "").trim());
      });
      this.save(index);
    });
  }
